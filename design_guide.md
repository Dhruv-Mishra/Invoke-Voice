...
- Settings use unframed sections and one label per control. Fixed option sets of up to four choices use the shared pillbar below the heading, with native buttons, radio semantics and arrow-key selection. Longer lists retain native selects. Keep checkbox switches; the label and Space must both work. Dynamic lists such as work areas and discovered agents retain native selects.
...
- The route dialog shows configured model names, never a second editable model field. Text defaults are the managed local model (`ling-local` alias, currently Gemma 4 E2B), GPT-5.6 Sol (`gpt-5.6-sol`) and Gemini 3.8 Flash (`gemini-3.8-flash`); saved provider settings take precedence. Identifiers must be available in the user's provider account; live cloud availability is not validated by the UI tests.
- The dock keeps options, mute, microphone, end call and chat in fixed positions. Mute has no duplicate options switch. Its ring uses real captured/playback levels from the app-scoped audio graph, resets on disconnect/interruption and pauses when the document is hidden. Captions remain the primary conversational feedback.
- While a local voice turn generates, a single thought cloud (`.thought-cloud`, rendered by `VoiceHome`) grows out of the sprite into one of six side slots, never overlapping it; on narrow screens it sits above the sprite. It uses caption surface tokens, accent dots and trailing circles pointing at the sprite, and shrinks back when the answer starts.
- Use the shared application dialog for renderer alerts and confirmations so Electron does not fall back to native Windows prompts. Native dialogs are reserved for failures that occur before the renderer is available.
