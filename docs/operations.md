...
Local setup is opt-in and requires explicit download consent. It provisions Gemma through llama.cpp, English [NVIDIA Parakeet TDT 0.6B v2](https://huggingface.co/nvidia/parakeet-tdt-0.6b-v2) (CC-BY-4.0) as an INT8 ONNX export by default, and Kokoro in an isolated Python 3.12 environment. Multilingual Whisper Small INT8 and Moonshine Streaming Tiny Q4_K with Silero VAD are explicit alternatives. Saved recognizer choices are preserved.

Parakeet uses the [istupakov/parakeet-tdt-0.6b-v2-onnx](https://huggingface.co/istupakov/parakeet-tdt-0.6b-v2-onnx) export at revision `0bbb45a3365852604aef28b538a8f066f4ccaa85`: the INT8 encoder (652 MB), the INT8 decoder/joint network, the NeMo 128-bin mel preprocessor and the vocabulary, each hash-verified. It runs in the existing Whisper worker with the same Silero VAD, endpointing, push-to-talk and early-decoding behavior, using ONNX Runtime and NumPy that the Whisper dependency pack already installs, so no Python package changes. Decoding is greedy TDT over the actual utterance length instead of Whisper's fixed 30-second window. It is English-only: choose Whisper for other languages; `WHISPER_LANGUAGE` applies only to Whisper.
...
For existing Moonshine or Whisper installations, choose **Parakeet** under **Local speech** and complete setup; it downloads only the Parakeet model files. Parakeet and Whisper transcribe complete utterances rather than streaming partial words. `WHISPER_END_SILENCE_MS` and `END_SILENCE_MS` default to 1400 ms and are editable in Settings; saved or environment overrides are preserved. With Parakeet or Whisper, push-to-talk starts an explicit segment and releases it on commit, so an internal pause does not submit a partial request. Hands-free mode still uses VAD endpointing. Moonshine retains its own endpointing during push-to-talk. Clear **Local GGUF path** in Settings to override a legacy environment path and use managed Gemma after restart.

**Early decoding** (Parakeet and Whisper) overlaps recognition with the existing hands-free silence window. After 480 ms of silence, an idle decoder may begin one provisional transcription per utterance, using the same model and settings as the final decode. The result stays private until the normal endpoint accepts exactly the same PCM and utterance metadata. Resumed speech cancels reuse; the complete request is decoded normally. There is no earlier endpoint, provisional tool execution or provisional spoken answer. Push-to-talk and Moonshine are unchanged, and no additional model or dependency is needed.

Early decoding defaults to On. Set **Settings > Providers & keys > Local speech > Parakeet/Whisper early decoding** to Off, or `WHISPER_PREDECODE_MS=0`, then restart to restore endpoint-only decoding. Native inference already in progress cannot be forcibly preempted; cancellation is checked between stages/segments. Speculation is skipped while the decoder is busy, limited to one attempt per utterance, and cannot displace queued confirmed work. Mid-sentence pauses can waste CPU, and slower or heavily loaded devices may see higher latency. No learned endpoint detector is enabled.
...
Kokoro renders every phrase with about 0.3 s of leading and 0.45 s of trailing digital silence, so each phrase boundary used to add roughly 0.75 s of dead air, and the first phrase started 0.3 s late. The worker now keeps 20 ms before the first voiced sample and 300 ms after a sentence (120 ms after a clause). The first phrase still ends at the first clause or sentence boundary after 24 characters (at most 56) to start audio quickly. Later phrases wait for a whole sentence, up to the number of characters already queued for speech (56 to 160), and only then fall back to a clause or word boundary, so synthesis stays ahead of playback without splitting sentences into separately intoned fragments.

**Thinking lines.** While a local or hybrid voice turn generates, the session sends a `thinking` event every 5 seconds with a line from the current stage (analyzing, taking action, checking results, wrapping up; five wordings each, never the same line twice in a row) and an empty `thinking` event when answer text starts. [src/voice-thinking.mjs](../src/voice-thinking.mjs) owns the lines. Each line is queued as speech for the same response, but only when nothing else is waiting to play, so it always finishes before the answer and lines never stack. With local Kokoro, a line's audio is synthesized once per voice, speed and pitch and saved as JSON under `cache/thinking-speech` in the cache directory; later turns and launches replay it without Kokoro. Cloud speech providers synthesize each line normally. On the reference machine a line takes 0.29-0.43 s to synthesize the first time and plays for 1.07-1.67 s; an answer that becomes ready mid-line waits for the rest of it. Native hosted realtime voices do not use thinking lines.
...
The release GGUF has no multi-token-prediction layer. With `QWEN_MTP=on` (default), setup range-downloads only the `blk.40` nextn layer (528,857,088 bytes, SHA-256 `4c55971b8e821de511c6c1b4dc0c5c91f4b0a64e8bd893ef5a172dd8ec90e422`) from [unsloth/Qwen3.6-35B-A3B-MTP-GGUF](https://huggingface.co/unsloth/Qwen3.6-35B-A3B-MTP-GGUF) revision `5bc3e238d916f48a861bac2f8a1990a0e9b7e98d`, validates the source header (architecture, 40 blocks, no existing nextn layer), and writes `...-Q4_K_P-MTP.gguf` with `block_count=41` and `nextn_predict_layers=1`. The MTP layer comes from the base model, so abliteration may lower draft acceptance but cannot change verified output. The unsloth GGUF also contains its own full-size base weights, but those are the unmodified Qwen3.6 checkpoint, not the selected HauhauCS fine-tune, so only its MTP layer is used. The merged file contains every source tensor byte-for-byte and becomes the only Qwen model: without a local copy, setup streams the pinned download straight into it while hashing the stream (the 23 GB original never lands on disk); with a verified local copy it builds from that file. A managed original in the model directory is then deleted, while a user-supplied copy (`LocalVoiceStack/LLMs` or `QWEN_MODEL_PATH`) is left alone and may be removed manually. The receipt records only the merged file's size and mtime, so it stays ready without the original, and `QWEN_MTP=off` launches the same file without speculative decoding. The build needs about 25 GB free. `npm run models -- qwen` performs the same steps.
...
September 23 recognizer comparison on the same Threadripper, eight threads, INT8, English: across 263 LibriSpeech clean/other and synthetic app-command clips (57 also mixed with 10 dB SNR pink noise), Parakeet TDT 0.6B v2 scored 3.0% / 3.9% / 4.6% WER (clean / other / noisy clean) with a 0.21 s median decode, against 7.0% / 7.4% / 10.5% and 1.09 s for Whisper Small. On identical subsets, Whisper small.en, distil-small.en, distil-medium.en, medium.en, distil-large-v3.5 and large-v3-turbo were each less accurate than Parakeet and took 0.9-5.5 s per utterance; Parakeet v3 was less accurate on English. Through the production worker (`stt parakeet` versus `stt whisper`), push-to-talk release to final fell from 1.04-1.44 s to 0.08-0.38 s and hands-free finals from 1.50-2.55 s to 1.40-1.60 s after speech, with exact transcripts in both. In a six-turn synthetic end-to-end run, end of speech to first audible word fell from 4.62 s to 3.36 s (Gemma) and 4.11 s to 3.29 s (Qwen with MTP), with no playback gaps. Text normalization is simple (lowercase, punctuation removed), so absolute WER differs from published leaderboards; the comparison is relative.
