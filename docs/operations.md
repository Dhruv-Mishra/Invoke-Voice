...
For Whisper Small INT8, choose **Whisper** under **Local speech**, complete setup and restart. It transcribes complete utterances rather than streaming partial words. `WHISPER_END_SILENCE_MS` and `END_SILENCE_MS` default to 1400 ms and are editable in Settings; saved or environment overrides are preserved. With Whisper, push-to-talk starts an explicit segment and releases it on commit, so an internal pause does not submit a partial request. Hands-free mode still uses VAD endpointing. Moonshine remains an optional streaming recognizer and retains its own endpointing during push-to-talk. Clear **Local GGUF path** in Settings to override a legacy environment path and use managed Ling after restart.
...
The current Ling Quality acceptance run passed eight of eleven cases: status, corrected status request, empty and failed reads, unique/ambiguous/missing subject lookup, and conversation. External delegation, deletion lookup, and an explicit worker follow-up still failed. The read/change guard and six-call limit bound execution but do not make a small model's intent selection reliable. Use the task UI for follow-ups and deletion when precision is required. Comparisons with Spark, MiniCPM, Qwen3 4B and Qwen2.5 7B also found routing failures; changing models alone did not resolve them. These are known failures, not a passing end-to-end local-agent claim.
