...
const fields = Object.freeze([
  ...
  { key: 'LOCAL_STT_PROVIDER', label: 'Local speech recognition', description: 'Parakeet is English-only, more accurate and several times faster than Whisper Small on CPU. Choose Whisper for other languages.', group: 'Local speech', type: 'select', defaultValue: 'parakeet', options: [['parakeet', 'Parakeet TDT 0.6B (English, INT8)'], ['whisper', 'Whisper Small (INT8, multilingual)'], ['moonshine', 'Moonshine Tiny (opt-in)']] },
  ...
  { key: 'WHISPER_END_SILENCE_MS', label: 'Parakeet/Whisper pause before reply (ms)', group: 'Local speech', type: 'number', defaultValue: '1400', min: 200, max: 3000, restartRequired: true },
  { key: 'WHISPER_PREDECODE_MS', label: 'Parakeet/Whisper early decoding', group: 'Local speech', type: 'select', defaultValue: '480', options: [['480', 'On'], ['0', 'Off']], restartRequired: true },
  ...
  { key: 'LOCAL_LLM_PROFILE', label: 'Local language model', description: 'Qwen3.6 35B-A3B is an opt-in mixture-of-experts model: much stronger tool use, about 24 GB of RAM, one 24 GB model file, and slower first load. Run local setup after switching.', group: 'Local model', type: 'select', defaultValue: 'gemma', options: [['gemma', 'Gemma 4 E2B (default)'], ['qwen', 'Qwen3.6 35B-A3B MoE (opt-in)']], restartRequired: true },
  ...
  { key: 'QWEN_MTP', label: 'Qwen multi-token prediction', description: 'Speculative decoding with the model\'s own MTP layer: faster replies with identical output. Setup merges the 0.5 GB layer into the single Qwen model file.', group: 'Local model', type: 'select', defaultValue: 'on', options: [['on', 'On'], ['off', 'Off']], restartRequired: true },
  ...
  { key: 'LOCAL_EARLY_PREFILL', label: 'Early reply preparation', description: 'With Parakeet or Whisper and Regular routing, prepare the local model with the provisional transcript while the end of speech is confirmed. Saves roughly 0.1-0.3 s of prompt work per turn; answers are unchanged.', group: 'Local performance', type: 'select', defaultValue: 'on', options: [['on', 'On'], ['off', 'Off']] },
  ...
  { key: 'WHISPER_THREADS', label: 'Parakeet/Whisper recognition threads', group: 'Local performance', type: 'number', defaultValue: localThreadDefault(8), min: 1, max: 128, restartRequired: true },
