...
test('speech pause and local model settings override legacy environment only after restart', context => {
  ...
  const config = createRuntimeConfig({ dataDir: directory, env });
  assert.equal(config.snapshot().fields.find(field => field.key === 'WHISPER_PREDECODE_MS').value, '480');
  config.update({ values: { END_SILENCE_MS: '1400', WHISPER_END_SILENCE_MS: '1800', WHISPER_PREDECODE_MS: '0', LOCAL_LLM_PATH: '' } });
  assert.equal(env.END_SILENCE_MS, '800');
  assert.equal(env.WHISPER_PREDECODE_MS, undefined);
  assert.equal(config.snapshot().fields.find(field => field.key === 'WHISPER_PREDECODE_MS').pendingRestart, true);
  createRuntimeConfig({ dataDir: directory, env });
  assert.equal(env.END_SILENCE_MS, '1400');
  assert.equal(env.WHISPER_END_SILENCE_MS, '1800');
  assert.equal(env.WHISPER_PREDECODE_MS, '0');
  assert.equal(env.LOCAL_LLM_PATH, '');
  assert.throws(() => config.update({ values: { WHISPER_END_SILENCE_MS: '0' } }), /integer/);
  assert.throws(() => config.update({ values: { WHISPER_PREDECODE_MS: '1' } }), /unsupported/);
