...
import { createRuntimeConfig, llamaThreadDefault, localThreadDefault, qwenThreadDefaults } from '../src/runtime-config.mjs';
...
test('local runtime defaults match displayed settings and honor explicit overrides', () => {
  ...
  try {
    ...
    assert.equal(value('--threads'), llamaThreadDefault());
    ...
    assert.equal(llamaThreadDefault(32), '16');
    assert.equal(llamaThreadDefault(16), '8');
    assert.equal(llamaThreadDefault(8), '4');
    assert.equal(llamaThreadDefault(1), '1');
    assert.deepEqual(qwenThreadDefaults(32), { decode: '12', batch: '16' });
    assert.deepEqual(qwenThreadDefaults(16), { decode: '8', batch: '8' });
    const qwenPaths = { ling: 'qwen.gguf', lingMtp: 'qwen-mtp.gguf', receiptDir: path.join(dataDir, 'receipts') };
    const qwenArgs = localLlmArguments(qwenPaths, url, { LOCAL_LLM_PROFILE: 'qwen', LLAMA_PARALLEL: '2', LLAMA_CONTEXT: '4096', LLAMA_THREADS: '3' });
    const qwenValue = flag => qwenArgs[qwenArgs.indexOf(flag) + 1];
    assert.equal(qwenValue('-m'), 'qwen.gguf');
    assert.equal(qwenValue('--parallel'), '1');
    assert.equal(qwenValue('--ctx-size'), '8192');
    assert.equal(qwenValue('--threads'), qwenThreadDefaults().decode);
    assert.equal(qwenValue('--threads-batch'), qwenThreadDefaults().batch);
    assert.equal(qwenValue('--load-mode'), 'none');
    assert.equal(qwenValue('--flash-attn'), 'on');
    assert.equal(qwenArgs.includes('--spec-type'), false, 'MTP is only enabled for a verified accelerated model');
    assert.equal(defaults.LOCAL_LLM_PROFILE, 'gemma');
    assert.equal(defaults.QWEN_MTP, 'on');
    assert.equal(defaults.QWEN_THREADS, qwenValue('--threads'));
    assert.equal(defaults.QWEN_CONTEXT, qwenValue('--ctx-size'));
    const tunedQwen = localLlmArguments(qwenPaths, url, { LOCAL_LLM_PROFILE: 'qwen', QWEN_THREADS: '6', QWEN_THREADS_BATCH: '7', QWEN_CONTEXT: '16384' });
    for (const [flag, expected] of [['--threads', '6'], ['--threads-batch', '7'], ['--ctx-size', '16384']]) assert.equal(tunedQwen[tunedQwen.indexOf(flag) + 1], expected);
